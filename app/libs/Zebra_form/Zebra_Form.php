 ...
 *
 *  Requires PHP 4.2.0+ or PHP 5.0.2+ and jQuery 1.5.2+
 *
...
class Zebra_Form
{
    ...
    function Zebra_Form($name, $method = 'POST', $action = '', $attributes = '')
    {
        ...
        $this->form_properties = array(
            ...
            'name'                      =>  $name,
            'other_suffix'              =>  '_other',
            'process_path'              =>  'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST']. '/' . substr(trim(str_replace('\\', '/', substr(__FILE__, 0, -strlen(basename(__FILE__)))), '/'), strlen(str_replace('\\', '/', realpath($_SERVER['DOCUMENT_ROOT'])))) . '/process.php',
     ...
     *  </code>
     *
     *  To access the JavaScript object and use the pulic methods provided by it, use $('#formname').data('Zebra_Form')
    ...
    function doctype($doctype = 'html')
    {
        ...
        // we need the argument as lowercase
        ...
        switch ($doctype) {
            ...
            case 'xhtml':
                ...
                // set the doctype
                $this->form_properties['doctype'] = $doctype;

                break;

            // if invalid doctype specified
            default:

                // default the doctype to html
                $this->form_properties['doctype'] = 'html';
        }
     ...
     *                                  file or can be generated programatically by a callback function.
     *
     ...
     *
     *                                  There is a third method of generating the output and that is programatically,
    ...
    function render($template = '', $return = false)
    {
        ...
        if ($this->form_properties['has_upload'] === true) {
            ...
            $this->client_side_validation(array('process_path' => $this->form_properties['process_path']));
        ...
        foreach ($this->controls as $key => $control) {
                ...
                foreach ($control->rules as $rule => $properties) {
                    ...
                    switch ($rule) {
                        ...
                        case 'length':
                            ...
                            // the class name also contains min/max
                            $class .= '(' . $properties[0] . ',' . $properties[1] . ')';
            ...
            if (isset($attributes['type']) && $attributes['type'] == 'select' && !isset($attributes['multiple']) && isset($attributes['other'])) {
                ...
                // if PHP version is greater thatn 5.0.2
            ...
            if (isset($attributes['type']) && $attributes['type'] == 'text' && preg_match('/\bdate\b/i', $attributes['class'])) {
                ...
                foreach ($control->attributes as $attribute => $value) {
                    ...
                    if (in_array($attribute, $control->javascript_attributes) && $control->attributes[$attribute] != null) {
        ...
        $function_name = 'init_' . md5(strtolower($this->form_properties['name'] . time()));

        $output .=
            '<script type="text/javascript" defer="true">function ' . $function_name . '(){if(typeof jQuery=="undefined"||typeof jQuery.fn.Zebra_Form=="undefined"' .
            (isset($datepicker_javascript) ? '|| jQuery.fn.Zebra_DatePicker=="undefined"' : '') . '){setTimeout("' . $function_name . '()",100);return}' .
            (isset($datepicker_javascript) ? $datepicker_javascript : '') .  '$("#' . $this->form_properties['name'] . '").Zebra_Form(' .
            ($javascript_object_properties != '' ? '{' . $javascript_object_properties . '}' : '') . ')}' . $function_name . '()</script>';
                    ...
                    switch ($rule_name) {
                        ...
                        case 'date':
                            ...
                            if (
                                ...
                                // because we are going to use regular expressions, make sure we escape characters that
                                // would make sense as regular expression
                                ...
                                if (!empty($regexp)) {
                                    ...
                                    if (preg_match($format, preg_replace('/\s/', '', $attribute['value']), $segments)) {
                                        ...
                                        if ($valid) {
                                            ...
                                            if ($attribute['value'] == date($control->attributes['format'], strtotime($english_months[$original_month] . ' ' . $original_day . ', ' . $original_year))) {
                        ...
                        case 'required':
                            ...
                            if (
                                ...
                                ($attribute['type'] == 'select' && !isset($attribute['multiple']) && strcmp($control->submitted_value, $notSelectedIndex) == 0) ||
     ...
     *
     *                                          If the <b>preserve_aspect_ratio</b> property is set to TRUE and both the
