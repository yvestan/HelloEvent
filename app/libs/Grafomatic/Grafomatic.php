...
class Grafomatic {
    ...
    */
    public function getFormeDate($dateentree,$sortie='Y-m-d')
    {    
    ...
    * Gère la traduction
    *
    * @param    string       $str   Code de traduction
    * @access   public
    * @return   string
    */
    public static function __($str)
    {
        ...
        if(!empty($GLOBALS['translate_config'][$str])) {
            return $GLOBALS['translate_config'][$str];
        } else {
            return $str;
        }
    ...
    /** Créer un pdf
    * 
    * Créer un pdf à partir d'un template
    *
    * @param    string       $template   Chemin vers le template
    * @param    string       $filename   Si on veut écrire dans un fichier
    * @param    array       $keywords   Chaines de remplacement
    * @param    string       $raw   Type de sortie
    * @access   public 
    * @return   string
    */
    public static function createPdf($template='pdf_template.html',$filename=null,$keywords=array(),$load='raw')
    {
        ...
        if(!empty($keywords)) {
            foreach($keywords as $key=>$value) {
                $patterns[] = "/{__\(".$key."\)__}/imSU";
                $values[] = $value;
            }
            $html = preg_replace($patterns, $values, $html);
        }
    ...
    /** Créer un message
    * 
    * Créer un message à partir d'un fichier
    *
    * @param    array       $keywords   Chaines de remplacement
    * @param    string       $orig_file   Chemin vers le template
    * @param    bool       $decode  Décoder les caractères
    * @param    bool       $suppr_empty_tags   Supprimer les tages inconnu
    * @access   public 
    * @return   string
    */
    public static function setMessage($keywords,$orig_file,$decode=false,$suppr_empty_tags=false)
    {
        ...
        // supprimer les tags restant
        if($suppr_empty_tags) {
            $output_content = preg_replace('/{__\(.*\)__}/imSU', '', $output_content);
        }
