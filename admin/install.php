...
<html>
    ...
    <h1 id="titre">Configuration de HelloEvent</h1>
<?php
...
require HE_APP_PATH.'/../app/libs/Zebra_form/Zebra_Form.php';

// utilities
require HE_APP_PATH.'/../app/libs/Grafomatic/Grafomatic.php';

// langue
require HE_APP_PATH.'/../app/languages/french.php';

// il y a déjà un fichier de configuration
if(file_exists(HE_APP_PATH.'/../app/config/config.inc.php')) {
    $html_errors['write_file'] = Grafomatic::__('write_file');
}

// tester le droit d'écrire dans le répertoire de configuration
if(!is_writable(HE_APP_PATH.'/../app/config/')) {
    $html_errors['write_file_no'] = Grafomatic::__('write_file_no');
}

// Check if PHP > 4.3
if(empty($html_errors)) {
    function he_check_php_version() {
        return ( version_compare( '5', phpversion() ) <= 0 );
    }
    // tester la version de PHP
    if(!he_check_php_version()) {
        $html_errors['old_php'] = Grafomatic::__('old_php');
    } else {
        $html_success['php_ok'] = Grafomatic::__('php_ok');
    }
}
...
if(empty($html_errors)) {
    if(!extension_loaded('pdo_mysql')) {
        $html_errors['no_pdo'] = Grafomatic::__('no_pdo');
    } else {
        $html_success['pdo_ok'] = Grafomatic::__('pdo_ok');
    }
}
...
// generate fields
if(empty($html_errors)) {
    foreach($fields_configuration as $f=>$v) {
        if($v['active']!==false) {
            // type
    ...
    // validation
    $form->add('submit', 'btnsubmit', Grafomatic::__('save_config'));
}

// validate the form
if ($form->validate() && empty($html_errors)) {
    ...
    try {
        $db = new PDO('mysql:host='.$values['he_sql_host'].';dbname='.$values['he_sql_db'], $values['he_sql_user'], $values['he_sql_pass']);
        $html_success['success_connect_db'] = Grafomatic::__('success_connect_db');
    } catch (PDOException $e) {
        $html_errors['error_db'] = Grafomatic::__('error_db').$e->getMessage();
    }

$sql_queries['create_country'] = " 
CREATE TABLE IF NOT EXISTS `helloevent_country` (
...
$sql_queries['insert_country'] = "INSERT INTO `helloevent_country` (`id_country`, `alpha2`, `alpha3`, `langCS`, `langDE`, `langEN`, `langES`, `langFR`, `langIT`, `langNL`) VALUES
...
$sql_queries['create_subscribe'] = "
CREATE TABLE IF NOT EXISTS `helloevent_subscribe` (
  ...
  `business_company` varchar(150) DEFAULT NULL,
  `business_function` varchar(150) DEFAULT NULL,
  `business_email` varchar(150) DEFAULT NULL,
  `business_mobile` varchar(25) DEFAULT NULL,
  `business_address` text,
  `business_zip_code` varchar(25) DEFAULT NULL,
  `business_city` varchar(150) DEFAULT NULL,
  `business_country` varchar(2) DEFAULT NULL,
  `business_website` varchar(250) DEFAULT NULL,
  `shipping_address` text,
  `shipping_zip_code` varchar(25) DEFAULT NULL,
  ...
  `badge` varchar(250) DEFAULT NULL,
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='HelloEvent Subscribe';";

$sql_queries['add_fk'] = "
ALTER TABLE `helloevent_subscribe`
  ADD CONSTRAINT `helloevent_subscribe_ibfk_1` FOREIGN KEY (`id_country`) REFERENCES `helloevent_country` (`id_country`) ON DELETE SET NULL;";
    ...
    foreach($sql_queries as $k=>$v) {
        if(empty($html_errors)) {
            try {
                $result = $db->exec($v); 
                $html_success['create_sql'] = Grafomatic::__('create_sql');
            } catch (PDOException $e) {
                $html_errors['create_sql_no'] = Grafomatic::__('create_sql_no').$e->getMessage();
            }
        }
    }
    ...
    if(empty($html_errors)) {
        // récupèrer la config
        $config_file = file_get_contents(HE_APP_PATH.'/../app/config/config-example.inc.php');
        // parcourir et créer le patterns de remplacement
        foreach($values as $f=>$v) {
            $patterns[] = '#define\(\''.strtoupper($f).'\'\, \'(.*)\'\)#imSU';
            $configs[] = 'define(\''.strtoupper($f).'\', \''.$v.'\')';
        }
        $config_file = preg_replace($patterns, $configs, $config_file);
        // renomer le fichier config-example.inc.php
        //rename(HE_APP_PATH.'/../app/config/config-example.inc.php', HE_APP_PATH.'/../app/config/config.inc.php');
        copy(HE_APP_PATH.'/../app/config/config-example.inc.php', HE_APP_PATH.'/../app/config/config.inc.php');
        // mettre la configuration dans le fichier
        file_put_contents(HE_APP_PATH.'/../app/config/config.inc.php', $config_file);
    }
...
?>
    ...
    <?php if(!empty($html_errors)): ?>
        <div class="Zebra_Form">
            <div class="error">
                <div class="container">
                <?php foreach($html_errors as $k=>$v): ?>
    ...
    <?php if(!empty($html_success)): ?>
        <div id="checked_install">
            <ul>
            <?php foreach($html_success as $k=>$v): ?>
                <li><?php echo $v; ?></li>
            <?php endforeach; ?>
            </ul>
        </div>
    ...
    <?php if(!empty($error_form)): ?>
        <div class="Zebra_Form">
            <div class="error">
                <div class="container">
                    <span><?php echo Grafomatic::__('error_configuration'); ?></span>
                </div>
            </div>
        </div>
    <?php endif; ?>
    ...
    <?php if(!empty($success_form)): ?>
        <div id="success"><?php echo Grafomatic::__('configure_ok'); ?></div>
    <?php endif; ?>

    <?php if(empty($success_form) && empty($html_errors)): ?>

        <h2 class="titre_bleu_16"><?php echo Grafomatic::__('configuration_install'); ?></h2>
        <p style="font-style: italic;"><?php echo Grafomatic::__('required_fields'); ?></p>
